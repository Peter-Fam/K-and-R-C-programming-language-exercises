...
int main(int argc, char **argv) {
    ...
    while (--argc > 0) {
        if (argc == 2) {
            ...
            if (entab <= 0 || col >= 0) {
                printf(
                    "Error: invalid argument\nPlease, provide correct n and m "
                    "where n > 0 and m < 0");
                return -1;
            }
            col = -col;
    ...
    while ((len = get_line_entab(line, MAXLINE, entab, col)) > 0) {
        printf("\n%s\n%d\n", line, len);
    }
    return 0;
}

static int get_line_entab(char s[], int lim, int n_col, int n_1stcol) {
    ...
    while ((c = getchar()) != EOF && c != (int)'\n' && i < lim - 1) {
        if (c != (int)' ' && c != (int)'\t') {
            if ((blanks > 0) &&
                ((screen_index % n_col == 0) ||
                 (screen_index >= n_1stcol && screen_index % n_1stcol == 0))) {
                s[i++] = '\t';
                blanks = 0;
            } else {
                while (blanks > 0) {
